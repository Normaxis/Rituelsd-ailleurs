    ...
    if selected_view == 'user':
        for user in users:
            ...
            for slot in slots:
                ...
                label = {'present': 'Presence', 'off': 'Repos', 'holiday': 'Conge', 'training': 'Formation'}.get(slot.status, slot.status)
                block = _block(slot.status, label, slot.note or 'Planning equipe', start_at, end_at, selected_date, slot=slot)
                if block:
                    blocks.append(block)
            for appointment in appointments:
                ...
                if block:
                    blocks.append(block)
            blocks.sort(key=lambda item: (item['top'], 0 if item['kind'] == 'present' else 1))
            board_resources.append({'id': user.id, 'type': 'user', 'name': user.full_name, 'subtitle': user.role.label if user.role else 'Praticienne', 'blocks': blocks})
    else:
        for cabin in cabins:
            blocks = []
            for appointment in appointments:
            ...
            board_resources.append({'id': cabin.id, 'type': 'cabin', 'name': cabin.name, 'subtitle': cabin.cabin_type, 'blocks': blocks})
