...
def index():
    if request.method == 'POST':
        ...
        routine = Routine(name=request.form['name'], cabin_id=cabin_id, instructions=request.form.get('instructions',''))
...
def report(validation_id):
    validation = RoutineCompletion.query.get(validation_id)
    if not validation:
        abort(404)
    steps = [line.strip() for line in (validation.routine.instructions or '').splitlines() if line.strip()]
    ...
    if request.method == 'POST':
        ...
        if not user:
            ...
            for routine_id in selected_ids:
                ...
                if not existing:
                    db.session.add(RoutineCompletion(routine_id=routine_id, cabin_id=cabin.id, user_id=user.id, completed_on=today, status='done', note=request.form.get('note','')))
            db.session.commit()
